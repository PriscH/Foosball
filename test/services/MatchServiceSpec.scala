...
class MatchServiceSpec extends Specification {
  ...
  "An outright Winner" should {
    ...
    "result from 2-1, 2-0, 2-0" in {
      MatchService.calculateResult(6, List(6, 3, 2, 2)) mustEqual MatchResult.Result.Winner 
    }
    ...
    "result from 2-1, 2-1, 2-1" in {
      MatchService.calculateResult(6, List(6, 3, 3, 3)) mustEqual MatchResult.Result.Winner 
    }
  }
  ...
  "Nothing" should {
    ...
    "result from 2-1, 2-0, 2-0" in {
      MatchService.calculateResult(2, List(6, 3, 2, 2)) mustEqual MatchResult.Result.NoResult 
    }
    ...
    "result from 2-1, 2-1, 2-1" in {
      MatchService.calculateResult(3, List(6, 3, 3, 3)) mustEqual MatchResult.Result.NoResult 
    }
